...
def test_stalled_table_is_flagged_during_an_open_session(monkeypatch):
    ...
    out = _health(monkeypatch, [
        {"table": "option_bars", "present": True, "last_ts": _fresh(2),
         "rows_today": 900, "sessions": 500, "error": None},
        {"table": "chain_snapshots", "present": True, "last_ts": _fresh(400),
         "rows_today": 0, "sessions": 7, "error": None},
    ])
...
def test_quiet_tables_are_not_stale_outside_session_hours(monkeypatch):
    ...
    worthless."""
    out = _health(monkeypatch, [
        {"table": "chain_snapshots", "present": True, "last_ts": _fresh(600),
         "rows_today": 0, "sessions": 7, "error": None},
    ], nse=False, mcx=False)
...
def test_never_written_table_is_stale_when_session_open(monkeypatch):
    ...
    out = _health(monkeypatch, [
        {"table": "index_bias_history", "present": True, "last_ts": None,
...
def test_missing_table_is_reported_not_raised(monkeypatch):
    out = _health(monkeypatch, [
        {"table": "setup_flags", "present": False, "last_ts": None,
         "rows_today": 0, "sessions": 0, "error": "CatalogException: nope"},
    ])
    row = out["tables"][0]
...
def test_age_minutes_is_computed(monkeypatch):
    out = _health(monkeypatch, [
        {"table": "underlying_bars", "present": True, "last_ts": _fresh(7),
         "rows_today": 80, "sessions": 500, "error": None},
    ])
